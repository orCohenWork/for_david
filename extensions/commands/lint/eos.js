...
module.exports = async (args)=>{
    ...
    let stdout = await execPromise(`docker run -u $(id -u \$USER) -v $PWD:/contracts --rm -i tmuskal/eosio.cdt clang-format -style=LLVM -i contracts/eos/*/*.hpp contracts/eos/*/*.cpp`,{});
    try{
        var checksArr = [
            '*',
        ...
        stdout = await execPromise(`docker run -u $(id -u \$USER) -v $PWD:/contracts --rm -i tmuskal/eosio.cdt clang-tidy -checks=${checks} contracts/eos/*/*.cpp -p /contracts/contracts/eos  -- -std=c++17 -c -mllvm -fno-cfl-aa -O3`);
