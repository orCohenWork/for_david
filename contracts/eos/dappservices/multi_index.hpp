...
class sharded_hashtree_t{
    ...
    sharded_hashtree_t(uint64_t scope, uint32_t shards = 1024,uint32_t buckets_per_shard = 64){
        _shards = shards;
  ...
  private:
    ...
    std::vector<char> getBucket(bucket_t& sb) const{
        ...
        if(shardData == _shardbucket_table.end()){
            auto emptyDataHash = uri_to_ipfsmultihash(ipfs_svc_helper::setRawData(emptyentry()));
            ...
            auto new_shard_pointer = uri_to_ipfsmultihash(ipfs_svc_helper::setData(newBucketData));
        ...
        else
        {
            ...
            // get data 
            auto bucket_data = ipfs_svc_helper::getData<shardbucket_data>(ipfsmultihash_to_uri(shard_uri));
    ...
    void setBucket(bucket_t& sb, std::vector<char> data){
        ...
        // // commit bucket data
        auto new_pointer = uri_to_ipfsmultihash(ipfs_svc_helper::setRawData(data));

        
        // // replace 
        bucket_data.values[sb.bucket] = new_pointer;
        // // commit bucketdata
        auto new_shard_pointer = uri_to_ipfsmultihash(ipfs_svc_helper::setData(bucket_data));
        // // set shard in RAM
...
typedef h_const_iterator const_iterator;
 ...
 public:
      ...
      :_code(code),_scope(scope),primary_hashtree(_scope)
      {
      }
