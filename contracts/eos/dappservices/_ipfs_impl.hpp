...
static eosio::key256 ipfsmultihash_to_key256(ipfsmultihash_t ipfshash) {
    // skip 4 bytes 
...
static eosio::key256 uri_to_key256(std::string uri) {
...
TABLE ipfsentry {  
   ...
   key256 hash_key()const { return uri_to_key256(data_to_uri(data)); }  
};  
typedef eosio::multi_index<"ipfsentry"_n, ipfsentry, 
      indexed_by<"byhash"_n, 
      const_mem_fun<ipfsentry, key256, 
                               &ipfsentry::hash_key> 
                >> ipfsentries_t; 
...
typedef eosio::multi_index<"ipfsentry"_n, ipfsentry, \
      indexed_by<"byhash"_n, \
                 const_mem_fun<ipfsentry, key256, \
                               &ipfsentry::hash_key> \
                >> ipfsentries_t; \
static std::vector<char> getRawData(std::string uri){  \
    ...
    if(existing == cidx.end())  \
        ...
        svc_ipfs_cleanup(uri); \
    return existing->data;  \
}  \
static std::string setRawData(std::vector<char> data){  \
    auto _self = name(current_receiver()); \
    ipfsentries_t entries(_self, _self.value);  \
...
template<typename T>  \
static T getData(std::string uri){  \
    return eosio::unpack<T>(getRawData(uri)); \
}  \
template<typename T>  \
static std::string setData(T  obj){  \
    return setRawData(eosio::pack<T>(obj));  \
}   \
SVC_RESP_IPFS(warmup)(uint32_t size,std::string uri,std::vector<char> data){ \
    auto _self = name(current_receiver()); \
    ipfsentries_t entries(_self, _self.value);  \
    auto currentUri = data_to_uri(data); \
    ...
    auto cidx = entries.get_index<"byhash"_n>(); \
    auto existing = cidx.find(uri_to_key256(uri)); \
    eosio_assert(existing == cidx.end(), "already warmed up");  \
    entries.emplace(_self, [&]( auto& a ) {  \
                a.id = entries.available_primary_key();  \
                a.data = data;  \
    }); \
} \
SVC_RESP_IPFS(cleanup)(uint32_t size, std::string uri){ \
    auto _self = name(current_receiver()); \
    ipfsentries_t entries(_self, _self.value);  \
    auto cidx = entries.get_index<"byhash"_n>(); \
    auto existing = cidx.find(uri_to_key256(uri)); \
...
SVC_RESP_IPFS(commit)(uint32_t size,std::string uri){\
} 