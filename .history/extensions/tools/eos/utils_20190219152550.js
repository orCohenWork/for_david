...
async function cleos(cmd, args = getDefaultArgs()) {
    ...
    const stdout = await execPromise(`${process.env.CLEOS || (haveLocalCleos ? 'cleos' : `docker run -u $(id -u \$USER) --name zeus-cleos -w=/root -v ${path.resolve('.')}:/root --rm --link zeus-keosd:keosd --link zeus-eosio:eosio -i eosio/eos-dev:v1.3.1 cleos --wallet-url http://keosd:8900`)} ${nodeosUrlArg} ${cmd}`, {});
    return stdout;
}
