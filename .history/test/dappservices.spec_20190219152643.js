...
async function claimrewards({ deployedContract, serviceName = 'ipfs', provider = "pprovider1", amount = "500.0000" }) {
    var model = (await loadModels("dapp-services")).find(m => m.name == serviceName);
    var service = model.contract;
    var contract = deployedContract.address;
    let servicesTokenContract = await deployedContract.eos.contract(dappServicesContract);
...
describe(`DAPP Services Provider & Packages Tests`, () => {
    ...
    it.only('claim rewards', done => {
        (async () => {
            try {
                var selectedPackage = 'test124';
                var selectedPackage2 = 'test421';
                var testContractAccount = "consumer2";
                var testContractAccount2 = "consumer3";
                var package_period = 20;
                console.log("dapp developer")
                var { testcontract, deployedContract } = await deployConsumerContract(testContractAccount);
                await deployServicePackage({ package_id: selectedPackage, package_period });
                await selectPackage({ deployedContract, selectedPackage });
                await stake({ deployedContract, selectedPackage });
                console.log("investor")
                var { testcontract2, deployedContract } = await deployConsumerContract(testContractAccount2);
                let deployedContract2 = deployedContract;
                let res = await deployServicePackage({ package_id: selectedPackage2, package_period });
                // console.log(JSON.stringify(res))
                await selectPackage({ deployedContract: deployedContract2, selectedPackage: selectedPackage2 });
                await stake({ deployedContract: deployedContract2, selectedPackage, amount: "250.0000" });
                await delaySec(package_period);
                await claimrewards({deployedContract,selectedPackage , provider:testContractAccount})
                // await deployedContract.eos.contract(dappServicesContract).claimrewards({

                //     provider: testContractAccount
                // })
                // await delaySec(package_period);
                // await invokeService(testContractAccount, testcontract);
                // await delaySec(package_period);
